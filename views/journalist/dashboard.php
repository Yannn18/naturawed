...
$pageTitle = "Inspiration Articles - NaturaWed";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="flex min-h-screen bg-white font-sans text-[#1a1a1a]">
    ...
    <main class="flex-1 p-12 overflow-y-auto bg-[#f8f9fa]">
        ...
        <header class="flex justify-between items-start mb-8">
            ...
            <a href="index.php?action=write_article" class="flex items-center gap-2 px-6 py-3.5 bg-[#2a3f24] text-white rounded-xl text-xs font-bold tracking-widest uppercase hover:opacity-90 transition-all shadow-lg hover:shadow-xl active:scale-95">
                ...
                <?php foreach ($myArticles as $article): ?>
                    <div class="group bg-white rounded-[2rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col">
                        ...
                        <div class="relative h-56 overflow-hidden bg-gray-100 shrink-0">
                            <img src="<?= htmlspecialchars($article['image_url'] ?: 'https://picsum.photos/600/400') ?>" 
                        ...
                        <div class="p-6 flex items-start justify-between flex-1">
                            <div class="pr-4">
                                ...
                                <p class="text-[10px] font-bold tracking-widest text-gray-400 uppercase">
                                    By <?= htmlspecialchars($article['author_name']) ?> &nbsp;•&nbsp; <?= date('d M Y', strtotime($article['created_at'])) ?>
                                </p>
                            </div>
        ...
        <?php else: ?>
            <div class="w-full bg-white rounded-[2.5rem] border border-gray-100 border-dashed p-16 flex flex-col items-center justify-center text-center shadow-sm">
                ...
                <a href="index.php?action=write_article" class="px-8 py-4 bg-[#2a3f24] text-white rounded-xl text-xs font-bold tracking-widest uppercase hover:opacity-90 transition-all shadow-lg active:scale-95">
                    Write First Article
                </a>
            </div>
        <?php endif; ?>
